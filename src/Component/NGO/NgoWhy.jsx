...
const RequirementCard = ({ icon, title, description, iconBgColor, iconColor }) => {
  return (
    <div
      className="flex flex-col p-6 bg-white rounded-xl shadow-md border border-gray-100 
                 transform transition duration-300 ease-in-out 
                 hover:shadow-2xl hover:-translate-y-2 hover:scale-105 hover:border-blue-200"
    >
      {/* Icon */}
      <div className={`flex items-center justify-center w-12 h-12 mb-4 rounded-full ${iconBgColor}`}>
        <span className={`text-2xl ${iconColor}`} role="img" aria-label={title}>
          {icon}
        </span>
      </div>
    ...
    <section className="py-16 bg-white rounded-2xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p className="text-base sm:text-lg text-gray-600 max-w-3xl mx-auto mb-12">
          NGO Registration ensures legal recognition and compliance in India. 
          It is mandatory for societies, trusts, and non-profits to operate lawfully. 
          Get registered with our expert consultants.
        </p>
